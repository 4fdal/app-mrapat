...
public class ScannAbsensiActivity extends AppCompatActivity {
    ...
    private TextView waktuAbsen, ketAbsen ;
        ...
        if (result != null) {
            if (result.getContents() != null) {
                try {
                    ...
                    String id_raker_qrcode = dataQrCode.getString("id");
                    String id_raker = getRaker().getString("id") ;
                    if(id_raker_qrcode != null && id_raker != null){
                        this.absenHandler(id_raker_qrcode, id_raker);
                    } else {
                        Toast.makeText(getApplicationContext(), "TIDAK ADA KODE YANG SESUI", Toast.LENGTH_LONG).show();
                    }
